...
<template>
  <header>
    <ul class="gnb">
      ...
      <li
        v-for="(props, key) in this.$store.state.cityData"
        v-bind:key="key"
        v-if="i != '인트로'"
      >
        <a href="#" v-on:click="chgData(key)">
          {{ key }}
        </a>
      </li>
    </ul>
...
export default {
  name: "TopArea", // 컴포넌트를 부르는 이름
  // 컴포넌트 데이터 설정하기 : 서브컴포넌트 만들 때 데이터 리턴함
  data() {
    return {
      // 1. 도시정보 객체 변수 리턴
      sdata: this.$store.state.cityData,
      // 2. 메뉴번호(처음에 다음 메뉴인 2번을 넣어준다)
      num: 2,
    };
  },
  // 메서드 생성 구역
