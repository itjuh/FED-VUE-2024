...
const store = new Vuex.Store({
    ...
    actions:{
        // 제이슨 데이터 로드하기 메서드
        initData(){ 
        // initData({commit}){  // 이건 무조건 실행하는 방법 -> 우리는 호출하는 방법으로 감
            // 액션스 메서드 전달값으로 {commit}을 쓰면
            // 뮤테이션 구역으로 바로 commit사용가능!

            // 제이슨 데이터 변수에 할당!
            const result = jsnData;
            console.log('db 데이터 actions :',result);

            // 데이터를 가져오고 난 후 
            // items변수에 데이터를 세팅하도록 구현 된 메서드인 mutations의 setData호출
            // 이렇게 하면 순차호출이 됨(비동기 처리)
            // ->> commmit(메서드명, 전달값);
